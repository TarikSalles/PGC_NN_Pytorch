...
class GNNUS_BaseModel(nn.Module):
    ...
    def forward(self, A_input, A_week_input, A_weekend_input, Temporal_input, Temporal_week_input,
        ...
        elu_activation_arma_conv = nn.ELU()  # Objeto de ativação ELU
        softmax_activation_arma_conv = nn.Softmax()  # Objeto de ativação Softmax
        ...
        out_temporal = elu_activation_arma_conv(self.out_temporal(Temporal_input,A_input)) #Ativação ELU chamada
        ...
        out_temporal = softmax_activation_arma_conv(arma_out_temporal(out_temporal,A_input)) #Ativação Sofmax chamada
        ...
        out_week_temporal = elu_activation_arma_conv(self.out_week_temporal(Temporal_week_input, A_week_input)) #Ativação ELU chamada
        ...
        out_week_temporal = softmax_activation_arma_conv(arma_out_week_temporal(out_week_temporal,A_week_input)) #Ativação Sofmax chamada
        ...
        out_weekend_temporal = elu_activation_arma_conv(self.out_weekend_temporal(Temporal_weekend_input, A_weekend_input)) #Ativação ELU chamada
        ...
        out_weekend_temporal = softmax_activation_arma_conv(arma_out_weekend_temporal(out_weekend_temporal,A_weekend_input)) #Ativação Softmax chamada
        ...
        out_distance = elu_activation_arma_conv(self.out_distance(Distance_input, A_input)) #Ativação ELU chamada
        ...
        out_distance = softmax_activation_arma_conv(arma_out_distance(out_distance, A_input)) #Ativação Softmax chamada
        ...
        out_duration = elu_activation_arma_conv(self.out_duration(Duration_input, A_input)) #Ativação ELU chamada
        ...
        out_duration = softmax_activation_arma_conv(arma_out_duration(out_duration,A_input)) #Ativação Softmax chamada
        ...
        out_location_location = elu_activation_arma_conv(self.out_location_location(Location_time_input, Location_location_input)) #Ativação ELU chamada
        ...
        out_location_location = softmax_activation_arma_conv(arma_out_location_location(out_location_location,Location_location_input))  #Ativação Softmax chamada

        out_location_time = self.out_location_time(Location_time_input) #Camada linear

        out_location_time = F.relu(out_location_time)  #Linha adicionada 09/04

        out_location_time = F.dropout(out_location_time, p=0.3) #Dropout
        Linear_Layer_Dense_Out_Location = nn.Linear(40, self.classes) #Camada linear

        out_location_time = softmax_activation_arma_conv(Linear_Layer_Dense_Out_Location(out_location_time))  #Ativação Softmax chamada


        out_dense = 2.0 * out_location_location + 2.0 * out_location_time #Multiplicação com pesos
        out_dense = softmax_activation_arma_conv(self.out_dense(out_dense))  #Ativação Softmax chamada
        ...
        out_gnn = softmax_activation_arma_conv(self.out_gnn(out_gnn))  #Ativação Softmax chamada
